...
| 項目 | 内容 |
|------|------|
| **通信** | LINE Platform ⇄ 自社システム間は HTTPS（TLS1.2 以上）で暗号化 |
| **Webhook 検証** | LINE 署名（`x-line-signature`）でリクエスト元を検証。なりすまし防止 |
| **トークン保管** | チャネルアクセストークン・シークレットはサーバー側の環境変数で管理。クライアント側に出さない |
| **保存範囲** | LINE User ID（不可逆ID）・表示名・メッセージ本文。電話番号や住所は LINE 側から取得不可 |
| **アクセス制御** | スタッフごとに権限分離（管理者 / 一般スタッフ）。対応中ロックで操作の重複を防ぐ |
| **画像・動画** | 受信メディアは自社ストレージに保存。一定期間後に自動削除を推奨 |
| **退会時** | LINEブロックされた場合は自動的に「停止顧客」へフラグ。データは論理削除で対応 |

> **個人情報保護法対応**: LINE経由のやり取りも顧客対応の一部として個人情報扱い。利用目的を明記し、商用利用範囲を超えた二次利用は行わない。
...
| 指標 | 導入前の典型値 | 導入後の目標 |
|------|---------------|-------------|
| **LINE予約見落とし件数** / 月 | 2〜5件 | 0件 |
| **初回返信までの平均時間** | 数時間〜翌日 | 数十分〜2時間以内 |
| **LINE → 予約転記の所要時間** | 2〜3分 / 件（手作業） | 5〜10秒 / 件（ワンクリック） |
| **新規顧客の獲得チャネル** | 把握できず | LINE経由を数値化 |
| **対応漏れによる予約逸失** / 月 | 数件 | ほぼ0 |

> LINE 経由予約は「お客様の負担が少ない」ため、対応速度を上げると **リピート率にも好影響**。導入後の3ヶ月でリピート率の変化を見ると効果が分かりやすいです。
...
| 項目 | 目安 | 備考 |
|------|------|------|
| **初期構築** | 個別見積 | LINE Messaging API 設定支援込み |
| **月額利用料** | 個別見積 | 店舗数・利用機能により変動 |
| **LINE公式アカウント利用料** | LINE社の規定 | フリープラン（月200通まで）／有料プランあり |
| **AI予約抽出（オプション）** | 個別見積 | 希望日時・メニューの自動抽出機能 |
| **運用保守（API仕様変更追従）** | 月額制 | LINE Messaging API の仕様変更に追従するための運用契約推奨 |

> LINE公式アカウントのメッセージ数上限を超える運用が想定される場合、**スタンダード〜プレミアムプラン**への切替を商談時に検討いただきます。
